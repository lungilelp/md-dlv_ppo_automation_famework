 ...
 * @package helpers
 */
public class ValidateCreditorTransactionInFlex extends BasePage {

    public ValidateCreditorTransactionInFlex(WebDriver driver) {
        super(driver);
    ...
    public void validateCreditorTransactionInFlexSingleTxn(String strCreditorAccountNumber, String strCurrentEndToEndId, String CreditorControlSum) throws IOException
    {
        ...
        //*****************************Get,Validate and store the record ***********************************************
        try {
            int i = 0;
            ...
            try{
                do
                {
                    ...
                    else if(dbDriver.contains("mysql") || dbDriver.contains("oracle"))
                    {
                        ...
                        strQueryGetCreditorTxnRecord = "SELECT DAT_TXN,DAT_VALUE,TXT_TXN_DESC,TXT_TXN_USER_DESC,REF_CHQ_NO,COD_DRCR,COD_CC_BRN_TXN,AMT_TXN,RAT_CCY,REF_TXN_NO,REF_USR_NO " +
                                "FROM Ch_nobook " +
                                "WHERE Cod_acct_no = '"+strCreditorAccountNumber+"' " +
                    ...
                    if (strCreditorTxnRecord != null)
                    {
                        ...
                        if(dbAmount == dbCreditorControlSum)
                        {
                            ...
                            /**INFO*/
                            ExtentTestManager.getTest().pass(MarkupHelper.createLabel("Status=PASS: Creditor Transaction Record from Flex DB below", ExtentColor.GREEN));
                            ...
                            String[][] data =
                                    {
                                            ...
                                            {"=========","========","=====","===========","==============","=========","=====","========","======","======","=============","==========","=========="},
                                            //Contents
                                            {strTxn_Date,strValue_Data,strCreditorAccountNumber,strDescription,strUser_Description,strCheque_No,strDr_Cr,strOrig_Brn,strAmount,strRunning_Total,strRef_Txn_No,strEndToEndId,strValidationStatus}};
                            ...
                            //Print out the results
                            /**INFO*/
                            ExtentTestManager.getTest().pass(MarkupHelper.createLabel("Status=FAILED: Creditor Transaction Amount is not equal to expected,see transaction from Flex DB below",ExtentColor.RED));
                        ...
                        System.out.println("==========Creditor Transaction Record FROM DB ==================================");
                        System.out.println("Info: Current Creditor Transaction Record " +strCreditorTxnRecord);
